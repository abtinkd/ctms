...
PSignNet TSignNet::CopyNet() const {
...
PSignNet TSignNet::GetBalSgnProbSubNet() {
	TVec<TEdgeI> PosEVec, NegEVec;
	PSignNet NewNet = TSignNet::New();
...
PSignNet TSignNet::GetMinEmbeddedSubNet (const int MinEmValue) {
	PSignNet thisNet = PSignNet(this);
	PSignNet NewNet = TSignNet::New();
	int addE = 0, totE = 0;
...
bool TSignNet::operator==(const TSignNet& SgnNet) const {
...
PSignNet TSignNet::LoadEpinionsModified(const TStr& FNm) {
...
PSignNet TSignNet::LoadSignedNet(const TStr& InFNm, const int& SrcColId, const int& DstColId, const int& SignColId) {
	TSsParser Ss(InFNm, ssfWhiteSep, true, true, true);
	PSignNet Graph = TSignNet::New();
...
PSignNet TSignNet::GenRewire(const int& NSwitch, TRnd& Rnd) const{
  const int Nodes = GetNodes();
  const int Edges = GetEdges();
  PSignNet NetPt = TSignNet::New();
  TSignNet& Network = *NetPt;
...
PSignNet TSignNet::GenRewire2Hash(const int& NSwitch, TRnd& Rnd) const{
	const int Nodes = GetNodes();
	const int Edges = GetEdges();
	PSignNet NetPt = TSignNet::New();
	TSignNet& Network = *NetPt;
...
PSignNet TSignNet::RewireNetwork2(const int &NSwitch, const bool separateHash) {
...
PSignNet TSignNet::RewireNetwork(const int &NSwitch) {
	PSignNet Net = GetEdgeSubNet(1,-1);
	TIntH NIdDatH;
...
PSignNet TSignNet::PermuteEdgeSigns2() {
	PSignNet Net = TSignNet::New();
...
int TSignNet::GetStrictSgnPermSets(PSignNet& Net, THash<TIntPr, TIntVPr>& PermutableGroups) {
	TVec<TIntPrPr> PermutableSets;
	PSignNet StandardNet = Net->CopyNet();
	...
	PSignNet PosSubNet = StandardNet->GetSignSubNet(1), NegSubNet = StandardNet->GetSignSubNet(-1);
...
//This function is dummy.. can be deleted
PSignNet TSignNet::PermuteEdgeSignsStrict2() {
	PSignNet Net = CopyNet();
...
int TSignNet::GetAllEdgeSgnPermSets(PSignNet& Net, TSignNet::TEdgeI& EI, TVec<TIntPrPr>& ESgnPermSets) {
	const int ESign = EI();
...
double TSignNet::GetEdgePosSgnProb(const int& SrcNdId, const int& DstNdId) {
	PSignNet thisNet = PSignNet((TSignNet *)this);
...
bool TSignNet::GetRndESgnPermSet(PSignNet& Net, TSignNet::TEdgeI& EI, TIntPrPr& ERndPermSet) {
	const int ESign = EI();
	TSignNet::TNode SrcBaseNd = Net->GetNode(EI.GetSrcNId()), DstBaseNd = Net->GetNode(EI.GetDstNId());
...
PSignNet TSignNet::PermuteEdgeSignsStrict() {
	PSignNet Net  = CopyNet();
...
void TSignNet::CompareNetWith(const PSignNet& PNet2) const {
	using namespace std;
	const TSignNet& Net2 = *PNet2();
	cout << "\n------- Comparing Neworks -------\n" << endl;
...
TVec<TIntTr> TSignNet::GetEmbDistrb() const {
...
bool TSignNet::IsSameTriad2(const PSignNet& Net1, const PSignNet& Net2) {
...
void TSignNet::GenTriadEquivClasses(TTriadEqClasH& TriadGroups, const bool BiDirEdgeSide, const bool Signed, const bool ZeroEdgeSide) {
	PSignNet Triad;
		...
		for (int j = 0; j < EdgeState.Len(); j++) {
			...
			for (int k = 0; k < EdgeState.Len(); k++) {
				...
				for (int i = 0; i < TriadIdV.Len() && TriadId == -1; i++) {
					if (TSignNet::IsSameTriad(TriadIdV[i], Triad) != -1) { TriadId=i; break; }
				}
				if (TriadId == -1) {
					PSignNet NewTriad = Triad->CopyNet();
	...
	for (int i = 0; i < TriadIdCntH.Len(); i++) {
		TriadGroups.AddDat(TriadIdV[TriadIdCntH.GetKey(i)]->GetTriadStr(), TPair<PSignNet, int>(TriadIdV[TriadIdCntH.GetKey(i)], TriadIdCntH[i]));
	}
	return;
}

// Returns the probability of Triad in its Equivalency Class
double TSignNet::GetTriadProb2(const double& PlusProb) const {
...
void TSignNet::CountSignedTriads2(const TStr& OutFNm) const {
	...
	for (int t = 0; t < SigEqClasses.Len(); t++) {
		int PlusE = 0;
		PSignNet TriadNet = SigEqClasses[t].Val1->CopyNet();
		PSignNet TriadNetUnsig = TriadNet->CopyNet(); TriadNetUnsig->SetAllEDat(1);
...
void TSignNet::CountSignedTriads3(const TStr& OutFNm) const {
	TTriadEqClasH NetSigTriDistrib;
	TTriadEqClasH PermSigTriDisrib, RewireSigTriDistrib;
	PSignNet ThisPt = PSignNet((TSignNet*) this), PermNet, RewiredNet;
	double NetAllTriads, PermAllTriads, RewAllTriads;
	PermNet = ThisPt->PermuteEdgeSigns2();
	RewiredNet = ThisPt->RewireNetwork2();
	
	if (NetSigTriDistrib.Empty()) {NetAllTriads = TSignNet::CalcNetTriadsDistrib(NetSigTriDistrib, ThisPt);}
	PermAllTriads = TSignNet::CalcNetTriadsDistrib(PermSigTriDisrib, PermNet);
	RewAllTriads = TSignNet::CalcNetTriadsDistrib(RewireSigTriDistrib, RewiredNet);
...
int TSignNet::CalcNetTriadsDistrib(TTriadEqClasH& NetSigTriDistrib, PSignNet& Net) {
	...
	for (TEdgeI EI = Net->BegEI(); EI < Net->EndEI(); EI++) {
		...
		for (int n = 0; n < NbrV.Len(); n++) {
			PSignNet Tri = Net->GetTriad(EI.GetSrcNId(), EI.GetDstNId(), NbrV[n]);
