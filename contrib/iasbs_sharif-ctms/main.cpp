...
void PrintNStats(const char NetName[], const PSignNet SignedNet) {
	cout << endl;
...
void RemoveSelfEdges(PSignNet& sNet) {
...
void LoadSNet(PSignNet& signedNet, const char FLocation[]) {
	//TFIn FIn(FileLocation); signedNet = TSignNet::Load(FIn);
	//signedNet = TSignNet::LoadEpinions(FileLocation);
	//signedNet = TSignNet::LoadEpinionsModified(FileLocation);
	signedNet = TSignNet::LoadSignedNet(FLocation);
	...
	for(bool b : BiDir)
		for(bool s : Signed)
			for(bool z : Zero)
			{
				...
				int totalTriads = 0;
				TSignNet::GenTriadEquivClasses(TriadGroups, b, s, z);
				if (totalTriads == 0) {
...
void TraverseNodeSignedNetwork(PSignNet signedNet) {
	int countLine = 1;
...
void TraverseEdgeSignedNetwork(PSignNet signedNet) {
...
*/
void GetPridictionEdges (const PSignNet& OrigNet, TIntTrV& outputEdges, const int MinEmb, const bool Balanced, const int MaxEmb = TInt::Mx) {
...
void runAllSignPredictionMethods(const PSignNet& Network, TIntTrV& Edges, const TStr outputFNm,
								 const bool SbNet, const int algorithmsEnabled = 0)
{
	...
	if(algorithmsEnabled == 4 || algorithmsEnabled == 0 || algorithmsEnabled == 6) {//run Heuristic Balance method
		...
		TEdgeSignPred heuristicPredition(Network);
...
void TestAnalysis() {
	PSignNet testNet, permutedNet, tempNet, rewiredNet, rewiredNet2, balancedNet;
...
PSignNet GetNet(const TIntTrV& EL) {
...
void CrossNetPredictionNOLrn(const char NN, const int MinEm = 1, const bool Bl = false, const bool useNewAlg = false) {
	...
	static PSignNet epinions, slashdot, wikipedia;
...
void crsbNt (int MinEm = 1, int nodes = 10) {
	PSignNet tstnt, abstrNt; LoadSNet(tstnt, FileNEpinions);
	tstnt->SortNIdById();
	abstrNt = TSignNet::New();
