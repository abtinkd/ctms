...
protected:
	...
	TIntV YV;
public:
	TSignPredictionNaive(const PSignNet& Net, const TStr NtNm, TIntTrV& selectedEdges) {
		NetName = NtNm;
		bNet = Net;
...
protected:
	...
	virtual TPredictionResult TrainTestUseNet(PSignNet& net, TIntV& TstIndxs) = 0;
	double RunValidation(const int NFold = 10, const bool alsoRunInSubNetMode = true);
	PSignNet GetNet(const TIntV& Indexes);
};
...
protected:
	...
	TPredictionResult TrainTestUseNet(PSignNet& net, TIntV& TstIndxs);
	void FitPredictionModel(const bool ScaleF = true, const bool Newton = true);
public:
	TSignPredictionLearn(const PSignNet& Net, const TStr NtNm) {
		NetName = NtNm;
		bNet = Net;
	...
	TSignPredictionLearn(const PSignNet& Net, const TStr NtNm, TIntTrV& selectedEdges) {
		NetName = NtNm;
		bNet = Net;
...
private:
	...
	static TChA GetTriadStr(const PSignNet& Nt, int srcId, int dstId, int nbrId, bool IsSigned = true);
	static double CalcEffectiveEsPosProb(const PSignNet& Nt);
...
protected:
	...
	TPredictionResult TrainTestUseNet(PSignNet& net, TIntV& TstIndxs);
public:
	TSignPredicNoLrn(const PSignNet& Net, const TStr NtNm) {
		CreateFeatureV();
		NetName = NtNm;
		bNet = Net;
		for (TSignNet::TEdgeI EI = Net->BegEI(); EI < Net->EndEI(); EI++)
	...
	TSignPredicNoLrn(const PSignNet& Net, const TStr NtNm, TIntTrV& selectedEdges) {
		CreateFeatureV();
		NetName = NtNm;
		bNet = Net;
		for (int i = 0; i < selectedEdges.Len(); i++) {focusedNetEdges.Add(selectedEdges[i]);}
	...
	//Get nets Theta
	static void GetTheta(const PSignNet& Net, THash<TChA, TFlt>& thta, TStr saveFileName = "");
